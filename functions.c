    ...
    if (jogadores) {
        ...
        if (ftell(jogadores) != 0){
            *n = ftell(jogadores)/sizeof(JOGADOR);
            // Se o arquivo existir e n�o estiver vazio, retorna quantos jogadores h�
...
int verificarRanking(JOGADOR player, FILE *jogadores, int n){
    ...
    for (i = 0; i < n; i++){
        if (lista[i].ranking == ranking){
            printf("\nRanking j� ocupado, selecionar outro\n");
            return 0;
        }
    } return 1;
}
...
void escreverJogador(FILE *jogadores, int n) {
    if(jogadores) {
        ...
        do{printf("CPF do jogador (somente os 11 d�gitos): ");
        fflush(stdin);
        gets(novoJogador->cpf);}while(verificarCPF(novoJogador) == 0);
        do{printf("G�nero do jogador [M/F]: ");
        ...
        printf("Processador da m�quina do jogador: ");
        gets(novoJogador->equip.processador);
        printf("Placa gr�fica da m�quina do jogador: ");
        gets(novoJogador->equip.placaGrafica);
        do{printf("Mem�ria RAM da m�quina do jogador (GB): ");
        scanf("%d", &novoJogador->equip.memRAM);}while(novoJogador->equip.memRAM < 0);

        // STATUS DO CAMPEONATO
        printf("\n===== Status do campeonato =====\n");
        do{printf("Vit�rias do jogador: ");
        ...
        do{printf("Quantidade de t�tulos do jogador: ");
        scanf("%d", &novoJogador->titulos);}while(novoJogador->titulos < 0);
        do{printf("Posi��o do jogador no ranking: ");
...
void lerJogador(JOGADOR lista[], int index) {
        ...
        printf("G�nero do jogador [M/F]: %c\n", lista[index].genero);
        printf("Estado Civil: ");
        ...
        printf("Processador da m�quina do jogador: %s\n", lista[index].equip.processador);
        printf("Placa gr�fica da m�quina do jogador: %s\n", lista[index].equip.placaGrafica);
        printf("Mem�ria RAM da m�quina do jogador (GB): %d\n", lista[index].equip.memRAM);

        // STATUS DO CAMPEONATO
        printf("\n===== Status do campeonato =====\n");
        calcularPontuacao(&lista[index]);
        printf("Pontua��o total do jogador: %d\n", lista[index].status.pontuacao);
        ...
        // CONQUISTAS
        printf("\n===== Conquistas do jogador =====\n");
        printf("Quantidade de t�tulos do jogador: %d\n", lista[index].titulos);
        printf("Posi��o do jogador no ranking: %d\n\n", lista[index].ranking);
    return;
}
...
void listarJogadoresAlfabetica(FILE *jogadores, int n) {
    if(jogadores) {
        ...
        printf("\n Digite o �ndice do jogador para fazer a leitura (0 para sair)\n>>> ");
        scanf("%d", &playerIndex);

        if (playerIndex == 0) {
            return;
        } if (playerIndex < 0 || playerIndex > n) {
            printf("Valor inv�lido. Considerado como 0. Retornando...\n");
            return;
        } else {
...
void alterarJogador(FILE *jogadores, int *n) {
    if(jogadores) {
        ...
        printf("\n=====\n Digite o �ndice do jogador para fazer a altera��o de dados (0 para sair)\n>>> ");
        scanf("%d", &playerIndex);
        ...
        if (playerIndex == 0) {
            ...
            printf("=====\nDigite a informa��o que deseja alterar (0 para cancelar):\n");
            printf("[1] Nome\n[2] Dia de nascimento\n[3] M�s de nascimento\n[4] Ano de nascimento\n[5] CPF\n");
            ...
            printf("[16] Placa gr�fica do computador\n[17] Mem�ria RAM do computador\n[18] Vit�rias no campeonato\n[19] Derrotas no campeonato\n[20] Empates no campeonato\n");
            printf("[21] T�tulos do jogador\n[22] Posi��o no ranking mundial\n[23] Deletar jogador\n>>> ");
            scanf("%d", &toChange);
            printf(">>> ");
            ...
            switch (toChange){
                ...
                case 23:
                    ...
                    if (a == 1){
                        ...
                        printf("Opera��o cancelada! Retornando...\n");
                        return;
                    }
...
void inserirResultado(FILE *jogadores, int n) {
    if(jogadores) {
        ...
        printf("\nInsira o �ndice do jogador desejado para alterar sua pontua��o (0 para voltar):\n");
        printf(">>> ");
        scanf("%d", &playerIndex);
        getchar();
        playerIndex--;
        ...
        if(playerIndex >= 0) {
            ...
            printf("\nPontua��o atual: %d", lista[playerIndex].status.pontuacao);
            printf("\n\nInserir \n"
                   "[1] Vit�ria\n"
                   "[2] Empate\n"
                   "[3] Derrota\n>>> ");
...
void buscarJogadorNome(FILE *jogadores, int n) {
    if(jogadores) {
        ...
        for (i = 0; i < n; i++){
            ...
            printf("N�o h� correspond�ncias.");
            return;
        } else {
            for (i = 0; i < j; i++){
                printf("[%d] %s\n", i+1, aux[i].nickname);
            }
            printf("\nDigite o �ndice do jogador para ver as informa��es (0 para voltar)\n>>> ");
            scanf("%d", &playerIndex);
            if (playerIndex < 1 || playerIndex > j){
                printf("Valor inv�lido. Considerando como 0. Retornando...\n");
            } else{
...
void buscarJogadorRanking(FILE *file) {
    if(file) {
        ...
        printf("\nInsira um n�mero para ver o jogador que est� em seu ranking\n\n");
        scanf("%d", &busca);
...
void mediaDeSeguidores(FILE *jogadores, int n) {
    if(jogadores) {
        ...
        printf("M�dia de seguidores dos jogadores do campeonato: %.3f\n", media);
        return;
